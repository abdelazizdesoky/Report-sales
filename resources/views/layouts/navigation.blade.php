<div class="flex flex-col h-full bg-slate-800 dark:bg-slate-900">
    ...
    @endphp
    <div id="brand-header" class="px-6 py-6 border-b border-slate-700/50 flex items-center gap-3 transition-all duration-300">
        @if($settings?->logo_path)
            <img src="{{ asset('storage/'.$settings->logo_path) }}" alt="Logo" class="w-10 h-10 object-contain rounded-lg shrink-0">
        @else
        ...
        @endif
        <div id="brand-text" class="min-w-0">
            <h2 class="text-lg font-black text-white leading-tight truncate">{{ $settings?->company_name ?? '' }}</h2>
            <p class="text-[10px] text-slate-400 font-bold uppercase tracking-widest truncate">{{ $settings?->description ?? '' }}</p>
        </div>
    </div>
    ...
    <nav class="flex-1 px-4 py-6 space-y-6 overflow-y-auto custom-scrollbar overflow-x-hidden">
        <!-- Main Section -->
        <div class="space-y-1">
            <a href="{{ route('dashboard') }}" class="sidebar-link {{ request()->routeIs('dashboard') ? 'sidebar-link-active' : '' }}" title="لوحة التحكم">
        ...
        <div class="space-y-1">
            ...
            @can('view users')
            <a href="{{ route('users.index') }}" class="sidebar-link {{ request()->routeIs('users.*') ? 'sidebar-link-active' : '' }}" title="إدارة المستخدمين">
            ...
            <a href="{{ route('role-permissions.index') }}" class="sidebar-link {{ request()->routeIs('role-permissions.*') ? 'sidebar-link-active' : '' }}" title="مصفوفة الصلاحيات">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>
                <span class="sidebar-text">مصفوفة الصلاحيات</span>
            </a>
            @endcan

            @can('view settings')
            <a href="{{ route('settings.edit') }}" class="sidebar-link {{ request()->routeIs('settings.*') ? 'sidebar-link-active' : '' }}" title="الإعدادات">
            ...
            @can('view users')
            <a href="{{ route('salesmen-sync.index') }}" class="sidebar-link {{ request()->routeIs('salesmen-sync.*') ? 'sidebar-link-active' : '' }}" title="إدارة المندوبين">
                <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                <span class="sidebar-text">إدارة المندوبين</span>
            </a>
            @endcan
        </div>
    ...
    <div class="px-4 py-4 border-t border-slate-700/50">
        <div class="flex items-center gap-3 p-3 rounded-xl bg-slate-700/30 hover:bg-slate-700/50 transition-colors cursor-pointer group">
            <div class="w-10 h-10 rounded-full bg-gradient-to-br from-emerald-400 to-cyan-500 flex items-center justify-center text-white font-bold text-sm shadow-lg shrink-0">
